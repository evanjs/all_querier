...
pub fn init_logging(verbose: bool) -> anyhow::Result<()> {
    let env_filter = if let Ok(rust_log) = std::env::var("RUST_LOG") {
        ...
        EnvFilter::try_new(
            "warn,allq_query=debug,allq_providers=debug,allq_wikidata=debug,allq_core=debug,allq_mal=debug,allq_pcgw=debug,allq_musicbrainz=debug,allq_jikan=debug,reqwest=debug,nu_plugin_all_querier=debug"
        )?
    } else {
